...
function draw() {
  if (state == 0) {
    ...
    textSize(35);
    text(
      "do you accept or refuse the meal?",
      20,
      100,
      windowWidth,
      windowHeight
    ...
    textContent =
      ...
      " " +
      Zutaten[rand2] +
      ", mix them very well in a big bowl, " +
      Verb[rand] +
      " some " +
      Zutaten[rand3] +
      " and " +
    ...
    for (let i = 0; i < foodPositions.length; i++) {
      Powder.spawnAtPosition(
        foodPositions[i].x,
        foodPositions[i].y,
        foodPositions[i].color
      );
    }

    for (let i = 0; i < liquidPositions.length; i++) {
      Liquid.spawnAtPosition(
        liquidPositions[i].x,
        liquidPositions[i].y,
        liquidPositions[i].color
      );
    }
    ...
    for (const [key, value] of Object.entries(liquidColors)) {
      if (currentWord.includes(key)) {
        ...
        while (currentPosX < currentX) {
          liquidPositions.push({
            x: Math.floor(currentPosX),
            y: currentY,
            color: value,
            name: key,
          });
...
class PixelGrain {
  ...
  static spawnAtPosition(x, y, color) {
    ...
    [
      ...
      if (random(0, 10) < 1 && PixelGrain.positions[pos] == null)
        new this(position, color);
    });
  }
...
class Powder extends PixelGrain {
  ...
  movePixel = (nextIndex) => {
    if (PixelGrain.positions[this.pos + nextIndex] == null) {
      ...
      PixelGrain.positions[this.pos] = null;
      this.clearPixel();
      PixelGrain.positions[this.pos + nextIndex].movePixel(
        -rowLenght + this.bias
      ) ||
        PixelGrain.positions[this.pos + nextIndex].movePixel(
          -rowLenght - this.bias
